...
module ApplicationHelper
  ...
  def pure_rails_pagination(total_count, current_page, base_path, per_page = 10)
    ...
    # 1. 「前へ」ボタンの生成
    if current_page > 1
      # パス（base_path）に対して、検索クエリとページ番号を安全に結合
      link_url = "#{base_path}?" + query_params.merge(page: current_page - 1).to_query
    ...
    (1..total_pages).each do |page|
      if page == current_page
        ...
        link_url = "#{base_path}?" + query_params.merge(page: page).to_query
        html += link_to(page.to_s, link_url, class: "inline-flex items-center justify-center w-9 h-9 text-sm font-medium text-gray-600 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 hover:text-blue-600 hover:border-blue-300 transition-colors")
      end
    end
